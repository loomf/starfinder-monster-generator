...
func (this *Creature) Complete(arrays map[string]map[string]Array, types map[string]Type, subtypes map[string]Subtype, skills []string, abilities []Ability) error {
    var err error
	err = this.ArraySpec.CompleteArrayType(arrays)
    if err != nil {
        return err
    }
	err = this.ArraySpec.CompleteCR(arrays)
    if err != nil {
        return err
    }
	err = this.TypeSpec.CompleteType(types, subtypes)
    if err != nil {
        return err
    }
	err = this.TypeSpec.CompleteSubtype(types, subtypes)
    if err != nil {
        return err
    }
	err = this.ModifierAssignment.Complete()
    if err != nil {
        return err
    }
	return nil
...
func (this *Creature) CompleteCR(validArrays map[string]map[string]Array) error {
    if this.CR == "" {
    ...
    array, validCR := arrayTypeArrays[this.CR]
    if !validCR {
        return fmt.Errord("Unknown CR %q\n", this.CR)
    }
    return nil
}

func (this *TypeSpec) CompleteType(validTypes map[string]Type, validSubtypes map[string]Subtype) error {
    if this.Type == nil {
        var err error
...
func (this *TypeSpec) CompleteSubtype(validTypes map[string]Type, validSubtypes map[string]Subtype) error {
    if this.Subtype == nil {
        var err error
